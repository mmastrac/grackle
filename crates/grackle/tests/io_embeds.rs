...
use grackle::db::graph::Graph;
use grackle::db::{Key, SiteDb};
...
fn load_err(dir: &Path) -> String {
    let cfg = grackle::config::Config::load(&dir.join("grackle.toml"));
    match cfg {
        Err(e) => format!("{e:#}"),
        Ok(cfg) => format!(
            "{:#}",
...
fn build_err(dir: &Path) -> String {
    let cfg = grackle::config::Config::load(&dir.join("grackle.toml")).expect("the config loads");
    let mut db = grackle_source::load(&cfg).expect("the site loads");
    let e = match grackle::build::render_site(&cfg, &mut db) {
...
fn text(out: &grackle::build::SiteOutput, url: &str) -> String {
    String::from_utf8(
        out.get(url)
...
fn a_strong_citation_produces_the_inputs_edge() {
    ...
    let fan: Vec<String> = Graph::of(&db)
        .fanout(&grackle::db::graph::Node::Input(Key::new(
            "assets/kite.png",
        )))
        .iter()
