...
use grackle::filter::{Filter, Row as _, Value};
...
fn site(who: &str, files: &[(&str, &[u8])]) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("grackle-io-join-{who}"));
...
fn built(dir: &Path) -> (grackle::build::SiteOutput, SiteDb) {
    let cfg = grackle::config::Config::load(&dir.join("grackle.toml")).expect("the config loads");
...
fn lands(r: &grackle::db::Row) -> bool {
    r.field("output") == Value::Bool(true)
}

fn keys(ks: &[grackle::db::Key]) -> Vec<String> {
    ks.iter().map(|k| k.to_string()).collect()
}

fn route<'a>(db: &'a SiteDb, url: &str) -> &'a grackle::db::Route {
    db.routes
        .get(&grackle::db::Key::new(url))
...
"#,
    ...
    let landless = Filter::parse("!output", &grackle::db::row_schema()).expect("`!output` parses");
    ...
    let lands = Filter::parse("output", &grackle::db::row_schema()).unwrap();
...
"#,
    ...
    let multi = Filter::parse("alternates", &grackle::db::row_schema()).expect("parses");
