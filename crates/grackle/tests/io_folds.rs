...
fn text(out: &grackle::build::SiteOutput, route: &str) -> String {
    String::from_utf8(
        out.get(route)
...
fn locs(out: &grackle::build::SiteOutput, route: &str) -> Vec<String> {
    text(out, route)
        .lines()
...
fn absent_from_without_a_fold_is_a_load_error() {
    ...
    let e = format!(
        "{:#}",
        grackle::config::Config::load(&cfg).expect_err("a listing must say what it lists")
    );
...
fn the_star_spelling_is_gone() {
    ...
    .unwrap();
    let cfg =
        grackle::config::Config::load(&cfg).expect("the config still parses — `*` is a VALUE");
    let e = format!(
        "{:#}",
