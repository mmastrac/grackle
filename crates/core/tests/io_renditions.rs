...
//!
//! claim about images is that nothing about them is special: an image is
...
//! parameters. says where the parameters come from, and it is the whole
...
fn a_rendition_is_an_output_of_its_input_with_the_parameters_it_was_made_with() {
    ...
    for (url, inputs, params) in &made {
        ...
        let (rebuilt, ext) = grackle_core::thumbs::render(&bytes, "png", *params)
...
/// anywhere said so. This is settled by what got built, no
...
fn two_pages_asking_different_widths_are_two_outputs_of_one_input() {
    ...
    // "the pull model is the garbage collector", one transform along.
    assert!(
        !renditions(&db)
            .iter()
