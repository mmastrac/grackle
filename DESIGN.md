...
**The column SURVIVES `kind`'s deletion, and what died is the engine's internal use of it** *(IO.md I13, the last item of the IO ledger)*. §3 of that document says the enum is a flattened product of independent facts and should dissolve into them. Measured item by item, it dissolves partly, and the census is worth stating because the three survivors survive for three different reasons:

| what | outcome | why |
|---|---|---|
| `kind == View` — eight tests in `build.rs`, `links.rs`, `trails.rs`, `load.rs`, `views.rs` | **deleted**, respelled as the `view` column (three were asking twice and went outright) | "which view materialized this output" is already a column, and a route's `kind` is `View` **iff** it is non-empty — the one equality the eight rewrites stand on, pinned by `io_kind.rs` |
| `Post` vs `Page` — the render dispatch, `page_bodies`, `search_pass`'s two doc arms | **survives**, structurally | "this scope's role is posts" is a statement about CONFIG; a row carries the scope's *name*, not its role (§5i's ruling on the three vectors). The two render in different passes from different body stores, and no fact on the output says which |
| `Static` vs `Object` | **survives, unread** | both are byte copies, they are one arm wherever anything dispatches on them, and that arm equals `!rendered` on all six corpus trees — measured. The only reader that tells them apart is the `kind` column itself |
| the **column**, its `Enum` domain, `explain`'s `kind` line, `query urls --kind` | **survive whole** | grack.com's `[routes.search]` and its `[profiles.drafts]` restatement filter `kind == "post"`, meaning the blog corpus — SCOPE MEMBERSHIP — and the route pool has no column for that |

The unlock is named rather than implied: the column can go when scope membership is expressible on the output pool (a collection column on routes, or the shell respelling of search) **and** Matt has taken the migration decision, which is a byte change to two live artifacts. Until both, `kind` is live config vocabulary, and a debug surface printing it is the surface rather than a fossil.
