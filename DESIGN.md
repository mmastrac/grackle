...
- **Locale IS an axis, exposed through the q53 interface** *(built 2026-07)*. It stays two rows paired by `by_logical` (this section is unchanged), but it is a FILE axis in the q53 vocabulary — each member owns a content file (`index.fr.md`), against theme's REUSE axis where members share one row — and so it answers the same interface: `?locale=fr` selects a member the way `?theme=ledger` does (via `by_logical`, since the member is a different file; the `.fr` suffix is the implicit value, `index.fr.md?locale=en` overrides it), and locale composes with another axis as the outer/prefix dimension, `/fr/{theme}/…`. A member with no file skips — the file axis's only honest "missing" answer, since a reuse axis already covers the always-present case. See q53 for the composition and the reuse/file distinction.
